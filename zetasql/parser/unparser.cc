...
void Unparser::visitASTGroupBy(const ASTGroupBy* node, void* data) {
  ...
  {
    Formatter::Indenter indenter(&formatter_);
    const std::vector<ASTIntLiteral*> int_grouping_items();
    const std::vector<ASTExpression*> other_grouping_items();
    ...
    UnparseVectorWithSeparator(int_grouping_items, data, ",");
    println();
    UnparseVectorWithSeparator(other_grouping_items, data, ",", true);
  }
}
